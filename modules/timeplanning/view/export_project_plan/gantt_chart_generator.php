...
$showLabels = 0; //show allocated human resources
$showWork = 0;
$sortByName = 0;
$showPinned = 0;
$showArcProjs = 0;
...
if ($project->project_end_date != "" && $project->project_end_date != "0000-00-00 00:00:00" && $project->project_start_date != "" && $project->project_start_date != "0000-00-00 00:00:00") {
...
//$project->setAllowedSQL($AppUI->user_id, $q);//commented due to bugs on ff and chrome
    ...
    if ($caller == 'todo') {
        ...
        if ($project_id) {
            $q->addWhere('task_project = ' . $project_id . " and pei.is_leaf=1");
        }
...
//$graph->scale->week->SetStyle(WEEKSTYLE_FIRSTDAY);
    ...
    if ($caller == 'todo') {
        $graph->scale->actinfo->SetColTitles(array($AppUI->_('Task name', UI_OUTPUT_RAW),
            ...
            $AppUI->_('Finish', UI_OUTPUT_RAW)), array(180, 50, 60, 60, 60));
    } else {
        $graph->scale->actinfo->SetColTitles(array($AppUI->_('Task name', UI_OUTPUT_RAW))); /* ,
          (($showWork == '1') ? $AppUI->_('Work', UI_OUTPUT_RAW) : $AppUI->_('Dur.', UI_OUTPUT_RAW)),
          $AppUI->_('Start', UI_OUTPUT_RAW),
          $AppUI->_('Finish', UI_OUTPUT_RAW)), array(230, 60, 60, 60)); */
    }

    $graph->scale->tableTitle->Set($projects[$project_id]['project_name']);
// Use TTF font if it exists
// try commenting out the following two lines if gantt charts do not display
    $graph->scale->tableTitle->SetFont(FF_CUSTOM, FS_NORMAL, 8);
...
// check day_diff and modify Headers
    ...
    if ($day_diff > 240) {
        ...
        $graph->ShowHeaders(GANTT_HYEAR | GANTT_HMONTH); //display week
...
//This kludgy function echos children tasks as threads
    ...
    function showgtask(&$a, $level = 0) {
        ...
        $_SESSION["gantt_arr"][] = array($a, $level);
...
//$p = &$projects[$project_id];
    ...
    for ($i = 0; $i < count(@$gantt_arr); $i++) {
        ...
        $name = "A." . $activitiesIdsForDisplay[$a['task_id']] . ". " . $a['task_name'];
        if ($locale_char_set == 'utf-8' && function_exists('utf8_decode')) {
            $name = utf8_decode($name);
        }
        $name = ((mb_strlen($name) > 80) ? (mb_substr($name, 0, 79) . '.') : $name);
    ...
    $graph->Add($vline);
    $graph->Stroke($imageName);
}else{
    echo $AppUI->_("LBL_PROJECT_DATES_MANDATORY_FOR_GANTT_GENERATION");
}