...
function clash_process() {
	...
	for ($i = 0; $i < $days_between; $i++) {
		if ($first_date->isWorkingDay()) {
			...
			for ($j = $start_time; $j <= $end_time; $j += 30) {
				$slot_count++;
				$slots[$i][] = array('date' => $first_date->format('%Y-%m-%d'),
									 'start_time' => $j,
	...
	foreach ($event_list as $event) {
		...
		list($syear, $smonth, $sday, 
			 $shour, $sminute, $ssecond) = sscanf($event['event_start_date'], 
												  '%4d-%2d-%2d %2d:%2d:%2d');
		list($eyear, $emonth, $eday, 
			 $ehour, $eminute, $esecond) = sscanf($event['event_start_date'], 
												  '%4d-%2d-%2d %2d:%2d:%2d');
