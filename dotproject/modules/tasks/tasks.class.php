    ...
    */
    ...
    function get_deps_max_end_date( $taskObj ) {
        ...
        if ($tracked_dynamics) {
            ...
            $sql = "SELECT MAX(task_end_date) FROM tasks WHERE task_id IN ($deps) AND task_dynamic IN ($track_these)";
        }
        ...
        foreach ($tarr as $user_id) {
            if (intval( $user_id ) > 0) {
                ...
                if (dPgetConfig("check_overallocation") && $perc > $alloc[$user_id]['freeCapacity']) {
                    ...
                    $sql = "REPLACE INTO user_tasks (user_id, task_id, perc_assignment) VALUES ($user_id, " 
                        . $this->task_id . ", $perc)";
                    db_exec( $sql );
                }
            }
    ...
    function getAllocation( $hash = NULL, $users = NULL ) {
        ...
        $sql = "SELECT u.user_id, "
            ...
            . " IFNULL((IFNULL(up.pref_value,$scm)-SUM(ut.perc_assignment)),up.pref_value),0)), '%]') AS userFC," 
            . " IFNULL(SUM(ut.perc_assignment),0) AS charge, u.user_username,IFNULL(up.pref_value,$scm) AS chargeMax," 
            . " IF(IFNULL((IFNULL(up.pref_value,$scm)-SUM(ut.perc_assignment)),up.pref_value)>0," 
            . " IFNULL((IFNULL(up.pref_value,$scm)-SUM(ut.perc_assignment)),up.pref_value),0) AS freeCapacity" 
    ...
    function getChildren() {
        $sql = "SELECT task_id FROM tasks WHERE task_id <> '".$this->task_id."' AND task_parent = '".$this->task_id."'";
...
class CTaskLog extends CDpObject 
{
    ...
    function canDelete( &$msg, $oid=null, $joins=null ) {
        ...
        if (is_array( $joins )) {
            ...
            $sql = "SELECT $select FROM ".$this->_tbl." $join WHERE $k = ".$this->$k." GROUP BY $k";
...
function showtask( &$a, $level=0, $is_opened = true, $today_view = false) {
    ...
    if (@$a['task_log_problem']>0) {
        ...
        $s .= "\n\t<td align='center'><a href=\"?m=tasks&a=view&task_id=" . $a['task_id'] . '&tab=1">'.$AppUI->_('Log').'</a></td>';
    } 
    else {
  ...
  //  $alt = strlen($a['task_description']) > 80 ? substr($a["task_description"],0,80) . '...' : $a['task_description'];
    ...
    if ($showEditCheckbox) {
        $s .= "\n\t<td align='center'>"
            ."<input type=\"checkbox\" name=\"selected_task[{$a['task_id']}]\" value=\"{$a['task_id']}\"/></td>";
    }
    $s .= '</tr>';
    echo $s;
}
