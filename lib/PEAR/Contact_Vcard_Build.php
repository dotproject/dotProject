...
//dP all these commented lines begin with //dP
//dP changes concern in particular PEAR error handling
//dP this is not the original Contact_Vcard_Build.php file.
//dP to get the original file: 	check pear.php.net or
...
class Contact_Vcard_Build {
    ...
    */
    ...
    function escape(&$text)
    {
        if (is_object($text)) {
           ...
           //dP return $this->raiseError('The escape() method works only with string literals and arrays.');
    ...
    */
        ...
        $iter = null)
    {
        ...
        if (! is_integer($iter) || $iter < 0) {
          ...
          //dP  return $this->raiseError("$iter is not a valid iteration number for $comp; must be a positive integer.");

        } else {

            $result = $this->validateParam($param_name, $param_value, $comp, $iter);

           //dP if (PEAR::isError($result)) {
           //dP     return $result;
           //dP } else {
    ...
    */
    ...
    function validateParam($name, $text, $comp = null, $iter = null)
    {
        ...
        if (preg_match('/[^a-zA-Z0-9\-]/i', $text)) {
         ...
         //dP   $result = $this->raiseError("vCard [$comp] [$iter] [$name]: The parameter value may contain only a-z, A-Z, 0-9, and dashes (-).");
            ...
            case 'ENCODING':
                if ($text != '7BIT' &&
                  ...
                  //dP  $result = $this->raiseError("vCard 2.1 [$comp] [$iter]: $text is not a recognized ENCODING.");
                } else {
                    $result = true;
                }
                break;
            ...
            case 'VALUE':
                if ($text != 'INLINE' &&
                    ...
                    $text != 'URL' &&
                    $text != 'VCARD') {
                  //dP  $result = $this->raiseError("vCard 2.1 [$comp] [$iter]: $text is not a recognized VALUE.");
                } else {
                    $result = true;
                }
                break;
            ...
            default:
              //dP  $result = $this->raiseError("vCard 2.1 [$comp] [$iter]: $name is an unknown or invalid parameter name.");
                break;
            }
    ...
    */
    ...
    function setVersion($text = '3.0')
    {
        ...
        //dP    return $this->raiseError('Version must be 3.0 or 2.1 to be valid.');
        } else {
    ...
    */
    ...
    {
        if (! is_integer($iter) || $iter < 0) {
         //dP   return $this->raiseError('LABEL iteration number not valid.');
        } else {
    ...
    */
    ...
    {
        if (! is_integer($iter) || $iter < 0) {
       //dP     return $this->raiseError('TEL iteration number not valid.');
        } else {
    ...
    */
    ...
    {
        if (! is_integer($iter) || $iter < 0) {
       //dP     return $this->raiseError('EMAIL iteration number not valid.');
        } else {
    ...
    */
    ...
    function fetch()
    {
       ...
       //dP     return $this->raiseError('VERSION not set (required).');
        }

        // FN component is required
        if (! is_array($this->value['FN'])) {
       //dP     return $this->raiseError('FN component not set (required).');
        }

        // N component is required
        if (! is_array($this->value['N'])) {
       //dP     return $this->raiseError('N component not set (required).');
        }
