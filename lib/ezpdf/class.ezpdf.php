...
function ezNewPage(){
  $pageRequired=1;
  if (!empty($this->ez['columns']) && $this->ez['columns']['on']==1){
...
//echo 'left='.$this->ez['leftMargin'].'   right='.$this->ez['rightMargin'].'<br>';
  ...
  if (!empty($pageRequired) && !empty($this->ez['pageHeight']) && !empty($this->ez['topMargin'])){
  ...
  } else if (!empty($this->ez['pageHeight']) && !empty($this->ez['topMargin'])) {
    $this->y = $this->ez['pageHeight']-$this->ez['topMargin'];
  } else {
    // boom...
    error_log("ezpdf: " . __FUNCTION__ . " is unable to process ez['pageHeight'] and/or ez['topMargin']");
  }
}
